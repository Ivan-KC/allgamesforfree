...
function App() {
	...
	const giveawaysGames = giveaways.filter(g => g.type === "Game").slice(0, 10);
	const rewards = giveaways.filter(g => g.type === "DLC").slice(0, 5);
	const betas = giveaways.filter(g => g.type === "Early Access").slice(0, 5);

	const getId = (type: string, id: number) => `${type}-${id}`;
